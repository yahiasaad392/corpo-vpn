...
export default function Dashboard() {
  ...
  const [timer, setTimer] = useState(() => {
    ...
    if (connectedAt) {
      const elapsed = Math.floor(
        (Date.now() - parseInt(connectedAt, 10)) / 1000,
      );
  ...
  useEffect(() => {
    if (!isAdmin && currentUser?.email) {
      fetch(
        `${POLICY_API}/my-policy?email=${encodeURIComponent(currentUser.email)}`,
      )
        .then((r) => r.json())
        .then((data) => {
  ...
  const handleToggle = useCallback(async (isRetry = false) => {
    if (typeof isRetry !== 'boolean') isRetry = false;
    setError(null);

    if (!isRetry && (status === "connected" || status === "connecting")) {
    ...
    if (isAdmin) {
      ...
      if (res.success) {
        ...
        if (!isRetry && res.error?.includes("Handshake did not complete")) {
          setError("Handshake failed. Auto-reprovisioning and retrying...");
          try {
            await fetch(`${VPN_API}/reprovision`, {
              method: "POST",
              headers: { "Content-Type": "application/json" },
            ...
            console.error("Auto-reprovision failed", e);
          }
        }
    ...
    const criticalFails = criticalIds.filter(
      (id) => results[id] && !results[id].pass,
    );
    const warningFails = warningIds.filter(
      (id) => results[id] && !results[id].pass,
    );

    // ── SEND ONE COMBINED NOTIFICATION for all failures ──
    const criticalLabels = criticalFails.map(
      (id) => ALL_CHECKS_MAP[id]?.label || id,
    );
    const warningLabels = warningFails.map(
      (id) => ALL_CHECKS_MAP[id]?.label || id,
    );
    ...
    if (criticalFails.length > 0) {
      ...
      setError(
        `Device non-compliant. ${criticalFails.length} critical check(s) failed. See sidebar for details.`,
      );
      setStatus("disconnected");
      return;
    }
  ...
  return (
    <div className="relative min-h-screen">
      ...
      <div
        className={`p-8 space-y-8 max-w-7xl mx-auto transition-all duration-500 ${showCompliance || isScanning ? "mr-80" : ""}`}
      >
        {/* Error Banner */}
        {error && (
          <div
            className={`flex items-start gap-3 p-4 rounded-2xl border animate-fade-in ${complianceResults && (userPolicy?.critical_checks || []).some((id) => complianceResults[id] && !complianceResults[id].pass) ? "bg-red-600/20 border-red-500/50" : "bg-red-500/10 border-red-500/30"}`}
          >
            <AlertTriangle size={18} className="text-red-400 mt-0.5 shrink-0" />
            <div>
              <p className={`text-sm font-semibold text-red-300`}>
                {complianceResults &&
                (userPolicy?.critical_checks || []).some(
          ...
          <div className="mt-12 text-center space-y-2">
            ...
            >
              {status === "connected"
                ? "Corpo Tunnel Active"
                : status === "connecting"
                  ? "Establishing Corpo Tunnel..."
                  : status === "error"
                    ? "Connection Failed"
                    : "Encrypted Tunnel Idle"}
            </h2>
            <p className="text-slate-500 font-medium">
              Target: <span className="text-cyan-500">{hqGateway.name}</span> •{" "}
              {hqGateway.gatewayIp}
            </p>
            {isAdmin && status === "disconnected" && (
              <p className="text-amber-400/60 text-xs font-semibold uppercase tracking-wider">
                Admin — Compliance bypassed
              </p>
            )}
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <StatsCard label="Protocol" value={hqGateway.protocol} icon={Lock} />
          <StatsCard
            label="Session Time"
        ...
        {status === "connected" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-in">
            ...
            <StatsCard
              ...
              value={liveStats.handshake}
              icon={ShieldCheck}
            />
            <StatsCard
              label="Endpoint"
              value={liveStats.endpoint}
              icon={Globe}
            />
          </div>
        )}
            ...
            <div className="flex items-center gap-8">
              <div className="text-center">
                ...
                <div className="flex items-center gap-1.5">
                  <div
                    className={`w-2 h-2 rounded-full ${status === "connected" ? "bg-neon-green animate-pulse" : "bg-slate-600"}`}
                  />
                  <span
                    className={`text-xs font-mono ${status === "connected" ? "text-neon-green" : "text-slate-500"}`}
                  >
