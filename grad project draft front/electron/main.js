...
async function checkAntivirusRunning() {
  ...
  return {
    ...
    detail:
      active.length > 0
        ? active.map((a) => a.displayName).join(", ")
        : "No active AV",
  };
}
...
async function checkRealtimeProtection() {
  ...
  return {
    pass: !!data.RealTimeProtectionEnabled,
    detail: data.RealTimeProtectionEnabled ? "Active" : "Disabled",
  };
}
...
async function checkFirewall() {
  ...
  return {
    pass: allOn,
    detail: allOn ? "All profiles enabled" : "Some profiles disabled",
  };
}
...
function checkSupportedWindows() {
  ...
  if (major >= 10 && build >= 22000)
    return { pass: true, detail: "Windows 11", label: "Windows 11" };
  if (major >= 10 && build >= 19041)
    return { pass: true, detail: "Windows 10 (20H1+)", label: "Windows 10" };
  if (major >= 10)
    return { pass: true, detail: "Windows 10", label: "Windows 10" };
  return {
    pass: false,
    detail: `Unsupported (Build ${release})`,
    label: `Build ${release}`,
  };
}
...
async function checkNoPublicWifi() {
  ...
  const publicNet = profiles.find(
    (p) => p.NetworkCategory === 0 || p.NetworkCategory === "Public",
  );
  const privateOrDomain = profiles.find(
    (p) =>
      p.NetworkCategory === 1 ||
      p.NetworkCategory === 2 ||
  ...
  if (publicNet)
    return {
      pass: false,
      detail: `Public network detected: ${publicNet.Name}`,
    };
  if (!privateOrDomain)
    return { pass: false, detail: "No private/domain network found" };
...
async function checkDomainJoined() {
  const data = await psJson(
    "Get-WmiObject Win32_ComputerSystem | Select-Object PartOfDomain,Domain",
  );
  if (!data) return { pass: false, detail: "Cannot query domain status" };
  return {
    pass: !!data.PartOfDomain,
    detail: data.PartOfDomain ? `Domain: ${data.Domain}` : "Not domain joined",
  };
}
...
async function checkNoOtherVPN() {
  const data = await psJson(
    "Get-NetAdapter | Where-Object { $_.InterfaceDescription -match 'VPN|TAP|TUN|Hamachi|WireGuard' -and $_.Status -eq 'Up' } | Select-Object Name,InterfaceDescription",
  );
  ...
  return {
    ...
    detail:
      others.length > 0
        ? `Active: ${others.map((a) => a.Name).join(", ")}`
        : "No conflicting VPN",
  };
}
...
async function checkWindowsUpdates() {
  ...
  try {
    ...
    return {
      pass: daysSince <= 30,
      detail: `Last update: ${daysSince} days ago`,
    };
  } catch {
    return { pass: false, detail: "Cannot parse update date" };
  }
}
...
function checkDiskSpace() {
  ...
  return {
    pass: freeGb >= 2,
    detail: `${freeGb} GB free RAM (proxy for storage)`,
  };
}
...
async function checkUSBStorage() {
  const data = await psJson(
    "Get-WmiObject Win32_DiskDrive | Where-Object { $_.InterfaceType -eq 'USB' } | Select-Object Caption,Size",
  );
  ...
  return {
    ...
    detail:
      devices.length > 0
        ? `USB: ${devices.map((d) => d.Caption).join(", ")}`
        : "No USB storage",
  };
}
...
async function checkLocalAdmin() {
  ...
  return {
    pass: !isAdmin,
    detail: isAdmin ? "Running as administrator" : "Standard user",
  };
}
...
function checkVPNClientOutdated() {
  ...
  return {
    pass: !!found,
    detail: found
      ? "Secure Tunnel engine installed"
      : "Secure Tunnel engine not found",
  };
}
...
function checkHostnameMismatch() {
  ...
  const isGeneric = generic.some(
    (g) =>
      hostname.toLowerCase().startsWith(g) && hostname.length <= g.length + 8,
  );
...
async function getDiskHealth() {
  ...
  return {
    pass: true,
    detail: disks.map((d) => `${d.FriendlyName}: ${d.HealthStatus}`).join(", "),
  };
}
...
async function getInstalledSoftware() {
  const result = await psRun(
    "Get-ItemProperty HKLM:\\Software\\Microsoft\\Windows\\CurrentVersion\\Uninstall\\* | Select-Object -ExpandProperty DisplayName -ErrorAction SilentlyContinue | Select-Object -First 10",
  );
...
async function getConnectedPeripherals() {
  const data = await psJson(
    "Get-PnpDevice -Status OK -Class 'USB' | Select-Object FriendlyName -First 10",
  );
...
ipcMain.handle("compliance:run", async () => {
  ...
  const [
    ...
    checkBrowserOutdated(),
    checkLastMalwareScan(),
    getBatteryLevel(),
  ...
  const results = {
    ...
    overall:
      antivirus_running.pass &&
      realtime_protection.pass &&
      firewall_enabled.pass &&
      supported_windows.pass,
  };

  console.log(
    "[Compliance] Scan complete. Overall legacy pass:",
    results.overall,
  );
  return results;
});
...
async function forceRemoveTunnel() {
  ...
  try {
    ...
    try {
      await runCmd(`sc stop WireGuardTunnel$${TUNNEL_NAME}`, 5000);
    } catch {}
    try {
      await runCmd(`sc delete WireGuardTunnel$${TUNNEL_NAME}`, 5000);
    } catch {}
  }

  // Clean up the conf file so it can't be accidentally restarted
  try {
    if (fs.existsSync(CONF_PATH)) fs.unlinkSync(CONF_PATH);
  } catch {}
}
  ...
  if (savedConfigExists) {
    try {
      ...
      if (savedConfig.privateKey) {
        ...
        savedConfig.privateKeyPreview =
          savedConfig.privateKey.substring(0, 5) + "...";
...
ipcMain.handle("vpn:connect", async (_event, clientConfig) => {
  ...
  if (!wgExe) {
    return {
      success: false,
      error:
        "Secure Tunnel engine is not installed. Please contact your administrator.",
    };
  }
  ...
  if (!privateKey || !clientIp) {
    return {
      success: false,
      error:
        "Missing private key or client IP. Go to Settings → Tunnel Configuration.",
    };
  }
  ...
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      ...
      console.warn(
        `[CorpoVPN] Tunnel install attempt ${attempt} failed: ${err.message}`,
      );
      if (attempt === 1) {
  ...
  if (!handshake.success) {
    ...
    if (wgDebug) {
      try {
        debugOutput = await runCmd(`"${wgDebug}" show all`, 5000);
      } catch {}
    }

    console.error(
      "[CorpoVPN] Handshake failed after 10 seconds. Rolling back tunnel...",
    );
